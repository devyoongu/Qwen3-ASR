...
"""
wav/sequential/ 하위의 서로 다른 음원 파일을 동시에 STT 처리하는 부하 테스트.

- CONCURRENT_REQUESTS 개의 워커가 각각 다른 음원으로 동시에 스트리밍 요청
- 음원별 최종 인식 결과와 latency를 출력하고 종합 성능 통계를 제공

사용법:
    python examples/example_qwen3_asr_vllm_streaming_multi_audio.py
"""
...
CONCURRENT_REQUESTS = 100  # 동시 요청 수 (WAV_DIR 내 파일 수 이하로 설정)
SHUFFLE = False         # True: 음원을 무작위 순서로 배정
...
def load_audio_files(wav_dir: Path, n: int, shuffle: bool) -> List[Tuple[str, np.ndarray]]:
    """WAV_DIR에서 n개의 파일을 로드해 (filename, wav16k) 리스트로 반환."""
    ...
    if len(paths) < n:
        raise ValueError(
            ...
            f"CONCURRENT_REQUESTS를 {len(paths)} 이하로 설정하세요."
        )
    if shuffle:
...
async def main_async(audio_files: List[Tuple[str, np.ndarray]]) -> None:
    print(f"동시 요청 수: {CONCURRENT_REQUESTS}, chunk: {STEP_MS}ms")
    print(f"음원 디렉터리: {WAV_DIR}")
    print("=" * 70)
    ...
    async with aiohttp.ClientSession(connector=connector) as session:
        tasks = [
            ...
            for i, (fname, wav16k) in enumerate(audio_files)
        ]
    ...
    print("\n" + "=" * 70)
    print("[종합 성능 통계]")
    print(f"  동시 세션 수       : {CONCURRENT_REQUESTS}")
...
def main() -> None:
    ...
    audio_files = load_audio_files(WAV_DIR, n=CONCURRENT_REQUESTS, shuffle=SHUFFLE)
    print(f"{len(audio_files)}개 파일 로드 완료\n")
    asyncio.run(main_async(audio_files))
